...
import { problemSchema } from '@/shared/utils/validators';
...
export async function GET(req: NextRequest) {
  return withAuth(req, async (_, userId) => {
    ...
    const { data, error } = await supabase
      .from('patient_problems').select('*').eq('patient_id', patient.id).order('created_at', { ascending: false });
...
export async function POST(req: NextRequest) {
  return withAuth(req, async (_, userId) => {
    ...
    const { data, error } = await supabase
      .from('patient_problems').insert({ ...parsed.data, patient_id: patient.id }).select().single();
