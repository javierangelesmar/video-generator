...
import { immunizationSchema } from '@/shared/utils/validators';
...
export async function GET(req: NextRequest) {
  return withAuth(req, async (_, userId) => {
    ...
    const { data, error } = await supabase
      .from('patient_immunizations').select('*').eq('patient_id', patient.id).order('date_administered', { ascending: false });
...
export async function POST(req: NextRequest) {
  return withAuth(req, async (_, userId) => {
    ...
    const { data, error } = await supabase
      .from('patient_immunizations').insert({ ...parsed.data, patient_id: patient.id }).select().single();
