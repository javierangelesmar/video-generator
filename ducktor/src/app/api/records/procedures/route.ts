...
import { procedureSchema } from '@/shared/utils/validators';
...
export async function GET(req: NextRequest) {
  return withAuth(req, async (_, userId) => {
    ...
    const { data, error } = await supabase
      .from('patient_procedures').select('*').eq('patient_id', patient.id).order('date_performed', { ascending: false });
...
export async function POST(req: NextRequest) {
  return withAuth(req, async (_, userId) => {
    ...
    const { data, error } = await supabase
      .from('patient_procedures').insert({ ...parsed.data, patient_id: patient.id }).select().single();
